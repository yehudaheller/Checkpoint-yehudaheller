...
void encryptFile(const char* input_filename, const char* output_filename, const char* key);
void decryptFile(const char* input_filename, const char* output_filename, const char* key);
...
int main() {
    ...
    if (db_file) {
     ...
     // Encrypt the file (using a hardcoded encryption key for simplicity)
    const char* encryptionKey = "myEncryptionKey";
    encryptFile("db_file.txt", "db_file.txt", encryptionKey);
...
void addNewStudent(struct Student** students, int* num_students, struct School* school) {
    if (*num_students >= LEN) {
        ...
        // Encrypt the updated data before saving it to the file
        const char* encryptionKey = "myEncryptionKey";
        encryptFile("db_file.txt", "db_file.txt", encryptionKey);
    }
}
...
int openFileAndProcessStudents(const char* filename, struct Student** students, int* num_students) {
    FILE* file = fopen(filename, "rb");  // Use "rb" mode to read the encrypted file as binary
    if (!file) {
...
// Function to calculate the average of students in a specific course and layer
double calculateAverage(struct Student* students, int num_students, int grade_num, int num_layers) {
    int totalGrade = 0;
    int count = 0;
...
void encryptFile(const char* input_filename, const char* output_filename, const char* key) {
    ...
    // Generate the AES key and initialization vector (IV)
    AES_KEY aesKey;
    ...
    for (int i = 0; i < numBlocks; i++) {
        ...
        memcpy(inputBlock, plaintext + i * AES_BLOCK_SIZE, AES_BLOCK_SIZE);
        AES_cbc_encrypt(inputBlock, outputBlock, AES_BLOCK_SIZE, &aesKey, iv, AES_ENCRYPT);
        fwrite(outputBlock, 1, AES_BLOCK_SIZE, outputFile);
        memcpy(iv, outputBlock, AES_BLOCK_SIZE);
    }
...
void decryptFile(const char* input_filename, const char* output_filename, const char* key) {
    ...
    unsigned char iv[AES_BLOCK_SIZE];
    fread(iv, 1, AES_BLOCK_SIZE, inputFile);
    ...
    unsigned char inputBlock[AES_BLOCK_SIZE];
    unsigned char outputBlock[AES_BLOCK_SIZE];
    while (fread(inputBlock, 1, AES_BLOCK_SIZE, inputFile) == AES_BLOCK_SIZE) {
        AES_cbc_encrypt(inputBlock, outputBlock, AES_BLOCK_SIZE, &aesKey, iv, AES_DECRYPT);
        fwrite(outputBlock, 1, AES_BLOCK_SIZE, outputFile);
        memcpy(iv, inputBlock, AES_BLOCK_SIZE);
    }
